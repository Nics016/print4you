...
class OrdersController extends Controller
{
    ...
    public function actionNew()
    {
        ...
        elseif (Yii::$app->user->identity->role == User::ROLE_EXECUTOR) {
            $records = Orders::find()
                ->where(
                    "order_status='proccessing' AND executor_id="
        ...
        elseif (Yii::$app->user->identity->role == User::ROLE_COURIER) {
            $records = Orders::find()
                ->where(
                    "order_status='proccessing' AND courier_id="
                    . Yii::$app->user->identity->id
                    . " AND location="
    ...
    public function actionCompleted()
    {
        ...
        elseif (Yii::$app->user->identity->role == User::ROLE_EXECUTOR) {
            $records = Orders::find()
                ->where(
                    "order_status='completed' AND executor_id="
                    . Yii::$app->user->identity->id
                );
        }
        // Курьер
        elseif (Yii::$app->user->identity->role == User::ROLE_COURIER) {
            $records = Orders::find()
                ->where(
                    "order_status='completed' AND courier_id="
                    . Yii::$app->user->identity->id
                );
        }
